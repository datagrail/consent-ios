...
- `setCcpaOptout(_:completion:)` and `getCcpaOptout()`: a first-class CCPA/CPRA "Do Not Sell or Share" choice (TRUST-2591). iOS has no native DNSMPI signal, so the host app is the source of truth. The flag is stored on the device and never changes a category. It is written through to the user's Universal Consent record when Universal Consent is enabled, `universalConsent.sync_optout` is on, `setUserIdentifier` has succeeded in this app session, and an explicit consent choice is stored. Otherwise it rides the next Universal Consent write.

### Changed

- **Behavior change (TRUST-2591):** Universal Consent writes now include `ccpa_optout`, set from the user's stored choice (`setCcpaOptout`) and gated by `universalConsent.sync_optout`. iOS used to omit the field. The value is never derived from a category or ATT. When a login finds a record, the record's `ccpa_optout` replaces the local flag. On a re-sync it is adopted only when `sync_optout` is on. `clearUserIdentifier()` resets the flag to `false` and `reset()` wipes it. Records written earlier are adopted as they are.

- `setUserIdentifier` login rule (TRUST-2902). The device now persists the user hash (never the raw identifier) of the identity it is bound to, cleared by `reset()` and `clearUserIdentifier()`. On a login (device unbound or bound to a different identity): if a record is stored it replaces local state (categories the record omits take their config default, never the prior local value; a record carrying no consent choice returns local state to neutral) and nothing from the device is written, even when a pre-login choice exists; if no record is stored, an explicit local choice (one the user saved on this device while it was not bound to a different identity) seeds the new record, and config defaults are never written. State left by a different bound identity is not written and local state returns to neutral. Once bound, local changes sync as before.
