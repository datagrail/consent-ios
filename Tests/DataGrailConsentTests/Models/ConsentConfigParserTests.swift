...
final class ConsentConfigParserTests: XCTestCase {
    ...
    func testParseGdprFranceConfig() throws {
        ...
        XCTAssertEqual(config.consentContainerVersionId, "72c28510-a607-4bfc-8eb9-512273b9c625")
        ...
        // Verify layout
        XCTAssertEqual(config.layout.id, "d5e6b99f-90bc-4124-9954-8ae4880599cd")
        XCTAssertEqual(config.layout.name, "Global Layout")
        XCTAssertEqual(config.layout.description, "Global Layout")
        XCTAssertEqual(config.layout.status, "published")
        XCTAssertTrue(config.layout.defaultLayout)
        XCTAssertFalse(config.layout.collapsedOnMobile)
        XCTAssertEqual(config.layout.firstLayerId, "1b4c5952-ab0b-4f6d-82f4-c28a430e9d59")
        XCTAssertNil(config.layout.gpcDntLayerId)

        // Verify consent layers
        XCTAssertEqual(config.layout.consentLayers.count, 2)
        XCTAssertNotNil(config.layout.consentLayers["33e8abaf-f967-44d8-8ad4-56c9f18028f6"])
        XCTAssertNotNil(config.layout.consentLayers["1b4c5952-ab0b-4f6d-82f4-c28a430e9d59"])
    ...
    func testParseCpraUsCaliforniaConfig() throws {
        ...
        // Verify essential category is always_on
        let essentialCategory = categoryElement?.consentLayerCategories?.first {
            $0.gtmKey == "dg-category-essential"
        }
        XCTAssertTrue(essentialCategory?.alwaysOn == true)

        // Verify language picker elements are parsed without error
        let defaultLayer = config.layout.consentLayers["1b4c5952-ab0b-4f6d-82f4-c28a430e9d59"]
        let languagePickers = defaultLayer?.elements.filter {
            $0.type == "ConsentLayerLanguagePickerElement"
        }
        ...
        // Verify default layer has open_tcf button action
        let tcfButton = defaultLayer?.elements.first {
            $0.buttonAction == "open_tcf"
        }
